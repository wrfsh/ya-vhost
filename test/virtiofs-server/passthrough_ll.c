		    ...
		    struct fuse_conn_info *conn)
{
	...
	if (lo->flock && conn->capable & FUSE_CAP_FLOCK_LOCKS) {
        ...
        /* TODO: limit max descriptor len by queue size depending on config */
        uint16_t max_write = sysconf(_SC_PAGESIZE) * (128 - 4);
        if (conn->max_write > max_write) {
		      ...
		      mode_t mode, struct fuse_file_info *fi)
{
	...
	fd = openat(lo_fd(req, parent), name,
		    (fi->flags | O_CREAT) & ~(O_NOFOLLOW | O_DIRECT), mode);
	if (fd == -1)
		return (void) fuse_reply_err(req, errno);
		    ...
		    off_t offset, struct fuse_file_info *fi)
{
	...
	buf.buf[0].pos = offset;
        ...
        /* TODO: implement virtio_send_data_iov */
#if 0
	fuse_reply_data(req, &buf);
#else
        char* databuf = malloc(size);
        ssize_t res = pread(fi->fh, databuf, size, offset);
        if (res == -1) {
            fuse_reply_err(req, errno);
	    fuse_log(FUSE_LOG_DEBUG, "lo_read(ino=%" PRIu64 ", size=%zd, "
	        "off=%lu) - errno %d (%s)\n", ino, size, (unsigned long) offset, errno, strerror(errno));
        } else {
            fuse_reply_buf(req, databuf, res);
        }
        free(databuf);
#endif
}
...
static unsigned current_log_level;
...
int main(int argc, char *argv[])
{
	...
	if (opts.show_help) {
        ...
        /*
         * Debug flag and log_level options have almost identical meaning beacuse
         * most of logging is gated by debug flag and the only used level in
         * these cases is FUSE_LOG_DEBUG.
         * But here we assume they can be used independently.
         */
        lo.debug = opts.debug;

        /* Zero is FUSE_LOG_EMERGE that isn't used by CLI, so it means unset state */
        if (opts.log_level != 0) {
	...
	if (lo.source) {
		...
		fuse_log(FUSE_LOG_ERR, "source is not set\n");
		exit(-1);
	}
	if (!lo.timeout_set) {
		switch (lo.cache) {
